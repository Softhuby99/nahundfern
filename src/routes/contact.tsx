...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Ob Fragen, Feedback oder Reiseinspiration — ich freue mich riesig, von dir zu lesen." },
      ...
      <section className="max-w-6xl mx-auto px-6 md:px-8 pt-12 pb-20 grid lg:grid-cols-2 gap-12">
        <div>
          <p className="font-script text-2xl text-primary flex items-center gap-2">
            Ich freue mich von dir zu hören. <Heart className="size-4 fill-primary/40 text-primary" strokeWidth={1.5} />
          </p>
          <h1 className="font-display text-5xl md:text-7xl font-semibold tracking-tight mt-3 leading-none">
            Lass uns in<br />Kontakt bleiben
          </h1>
          <p className="font-script text-xl md:text-2xl text-primary/80 mt-6 leading-snug">
            Ob Fragen, Feedback, Reiseinspiration oder einfach ein Hallo —
          ...
          <div className="paper-card p-4 mt-8 max-w-sm rotate-[-1.5deg]">
            <p className="font-script text-lg leading-snug">
              Ich lese jede Nachricht persönlich und antworte so schnell wie möglich.
              Danke, dass du Teil meiner Reise bist!
            </p>
          </div>

          <div className="mt-8 grid sm:grid-cols-2 gap-4">
            <a href="mailto:hallo@reisejournal.de" className="paper-card p-4 flex flex-col gap-1 hover:-translate-y-0.5 transition-transform">
              <Mail className="size-5 text-primary" strokeWidth={1.5} />
              <p className="font-display font-semibold">E-Mail</p>
              <p className="text-xs text-muted-foreground">Für Fragen, Feedback oder einfach zum Austauschen.</p>
              <p className="text-primary text-sm mt-1">hallo@reisejournal.de</p>
            </a>
            <a href="https://instagram.com" target="_blank" rel="noreferrer" className="paper-card p-4 flex flex-col gap-1 hover:-translate-y-0.5 transition-transform">
              <Instagram className="size-5 text-primary" strokeWidth={1.5} />
              <p className="font-display font-semibold">Instagram</p>
              <p className="text-xs text-muted-foreground">Folge mir für tägliche Eindrücke und kleine Abenteuer.</p>
              <p className="text-primary text-sm mt-1">@reisejournal</p>
            </a>
            <div className="paper-card p-4">
              <Mail className="size-5 text-primary" strokeWidth={1.5} />
              <p className="font-display font-semibold">Newsletter</p>
              <p className="text-xs text-muted-foreground">Neue Reiseberichte, Tipps & Inspiration direkt in dein Postfach.</p>
            ...
            <div className="paper-card p-4">
              ...
              <p className="text-xs text-muted-foreground">Du möchtest mit mir arbeiten oder ein Projekt vorschlagen?</p>
        ...
        >
          <div className="paper-card px-5 py-2 -mt-12 mx-auto w-max rotate-[-3deg]">
            <p className="font-script text-2xl flex items-center gap-2">
              Schreib mir <Heart className="size-4 fill-primary/40 text-primary" strokeWidth={1.5} />
            </p>
          </div>
          ...
          <div className="mt-6 space-y-5">
            ...
            >
              {sent ? "Nachricht gesendet ✓" : (<>Nachricht senden <Send className="size-4" strokeWidth={1.5} /></>)}
            </button>
            <p className="text-xs text-muted-foreground flex items-center gap-1.5">
              <Lock className="size-3.5" strokeWidth={1.5} /> Deine Daten werden vertraulich behandelt und nicht an Dritte weitergegeben.
            </p>
          </div>
        </form>
      </section>
